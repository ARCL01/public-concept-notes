...
def get_safe_note_path(note_id):
    ...
    target_path = (NOTES_DIR / f"{note_id}").resolve()
...
def show_home():
    ...
    for row in cursor.execute('SELECT id, name, uploadDateTime, fileType FROM uploads'):
        data.append({
            "noteName": row[1], 
            "noteId": row[0], 
...
def show_note(noteId=None):
    ...
    cursor.execute('SELECT fileType FROM uploads WHERE id = ?', (noteId,))
    if cursor.fetchone()[0] == 'PDF':
        return send_from_directory('notes',f'{noteId}',download_name=f'{noteId}.pdf')
...
def download_note(noteId=None):
    cursor = get_db().cursor()
    result = cursor.execute('SELECT name, fileType FROM uploads WHERE id = ?', (noteId,)).fetchone()
    if result is None:
        return abort(404)
    ...
    if result[1] == 'SVG':
        ...
        download_name = secure_filename(f"{result[0]}.pdf")
    if not download_name:
        download_name = "note.unknown"

    return send_from_directory('notes',f'{noteId}',download_name=download_name,as_attachment=True)
